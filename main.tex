...
of the resulting set $\cP'$ cannot increase or decrease
too much with respect to the size of $\cP$.

\begin{lemma}
  \label{lemma:partition}
  Given trees $T_1$ and $T_2$ with the corresponding set $\cP$, if $T_1'$ is the tree (isomorphic to $T_1$)
  resulting from the application of permutation $\pi$ of the labels of $T_1$, and $\cP'$ is the set obtained for $T_1'$ and $T_2$, then $|\cP| - 2|\pi|
  \le |\cP'| \le |\cP| + 2|\pi|$.
\end{lemma}

\begin{proof}
  Let $a$ be some label of $T_1$ which has been perturbed by
  permutation $\pi$, i.e., $\pi(a) = b \ne a$.  The new set $\cP'$ is obtained from $\cP$ by means of deletions,
  ...
  \item $c_{T_1}(a) \cap \cP_{(a,a)}$, that is the children that are not in some set of $\cP$. The new set $\cP'_{(b,a)}$ will be created with those children;
  ...
  Overall, for each label $a$, at most one set of $\cP$ will be created and at most one will be deleted.
  ...
  and two additions of sets in $\cP$ (and an unbounded number of replacements) for each label
...
\begin{proof}
  ...
  $T_1$, and$\cP'$ the set obtained for
  $T'_1$ and $T_2$.  By the construction of $\cP'$, the optimal number of link-and-cut operations to obtain
  $T_2$ from $T'_1$ is at least $|\cP'|$; we thus have that
  $d(T_1,T_2) \ge |\pi| + |\cP'| $. Moreover, Lemma
  \ref{lemma:partition} says that $|\pi|\ge \frac{|\cP|-|\cP'|}{2}$,
  thus $d(T_1,T_2)\ge \frac{|\cP|-|\cP'|}{2} + |\cP'| =
  \frac{|\cP|}{2} + \frac{|\cP'|}{2} \ge \frac{|\cP|}{2}$. Let $X=\bigcup_{\cP_{(x,y)}\in\cP}\cP_{(x,y)}$ be the set nodes whose parents in $\Tone$ and $\Ttwo$ are different. Now, since
  $T_1$ is binary, each set in $\cP$ consists of
  ...
  $x$). It follows that $|X|=d_{\ell}(T_1,T_2)\le 2\cdot|\cP|$,
