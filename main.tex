...
parent in each tree, each vertex $v\in \cX$ \murray{just a reminder to
  update this notation} belongs to exactly one set $\cP_{(u,w)}$.
...
We first show that deciding the rearrangement distance between two
...
\begin{figure}[ht]
  ...
  \caption[Reduction from 3D matching]{The trees $\Tone$ and $\Ttwo$
...
\begin{proof}
  ...
  3-dimensional matching of size $n$.\gdv{Figure 5 left tree is
    wrong. The leaves under $a'$ must be labeled by $t$, not
    $s$}\murray{fixed, in reverting to trees from CPM paper.  I prefer
    the style of the other ones, however}
...
partition.
At the same time, Lemma~\ref{lemma:partition} shows that each permutation has only a limited effect on the number of sets of the family partition. 

Let $\mathcal{X}$ be the set of labels $x$ such that $x\in \cP_{(a,b)}$ with $a\neq b$ or $x$ has  a child $y$ such that $x\in \cP_{(a,b)}$ with $a\neq b$. Such a set $\mathcal{X}$ is called the active set.

Our algorithm consists of showing that
$|\mathcal{X}| \le 2 |\cP|$ and that only labels in $\mathcal{X}$ can be involved in a permutation.
Finding
...
is the family partition associated with $T_{1}$ and $T_{2}$.  The next lemma states that each operation in an optimal sequence strictly decreases the size of the family partition; this allows us to bound the size of the search tree in terms of the size of the family partition.
