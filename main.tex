...
In this section we present a simple linear-time algorithm to compute the link-and-cut distance between two trees. The main technical device is called \emph{family partition}.

Given two trees $T_1$ and $T_2$, we can associate with $v$ the pair
$(p_{T_1}(v),p_{T_2}(v))$ of the parents of $v$ in the two trees.  Given a pair of (not necessarily distinct) nodes $u$ and $w$, we define
$\cP_{(u,w)}$ as the set $\{v ~:~ p_{T_1}(v) = u, ~ p_{T_2}(v) = w\}$. 
Since each vertex different from the root has exactly one parent in each tree, each vertex
$v\in \cX$ belongs to exactly one set $\cP_{(u,w)}$.  This fact leads to the following definition, which is illustrated in
...
Given two trees $T_1$ and $T_2$ each labelled by $[n]$, let $\cP$ be the family partition associated to with those trees, and let $X$ be the set of nodes $v$ such that $\lambda(p_{T_1}(v)) \neq \lambda(p_{T_2}(v))$. 
Then there is a linear-time algorithm that computes the shortest sequence of link-and-cut operations that transforms $T_1$ into $T_2$, and such a sequence consists of $|X|$ operations.
\end{lemma}
%
\begin{proof}
  ...
  Let $v$ be a node of $X$ such that no proper descendant of $v$ in $T_1$ belongs to $X$ --- if no such node $v$ exists, then $T_1$ and $T_2$ are isomorphic.
  Without loss of generality, $v\in \cP_{(u,w)}$, that is $p_{T_1}(v)=u$ and $p_{T_2}(v)=w$.

  If $w$ is not a descendant of $v$ in $T_1$, we can apply the $\edge{v}{u}{w}$ operation --- hence removing $v$ from $X$ --- and recurse on the resulting tree.

  Assume now that $w$ is a descendant of $v$ in $T_1$, and let $<v, x_1, \ldots, x_{q-1}, x_q = w>$ be the path of $T_1$ from $v$ to $w$. Since no proper descendant of $v$ in $T_1$ belongs to $X$, $p_{T_1}(x_i)=p_{T_2}(x_i)$ for each $i\le q$, that is $<v, x_1, \ldots, x_{q-1}, x_q = w>$ is also path of $T_2$ --- $v$ is an ancestor of $w$ in $T_2$. 
