...
Given two trees $T_1$ and $T_2$, we can associate with $v$ the pair
$(p_{T_1}(v),p_{T_2}(v))$ of the parents of $v$ in the two trees.
Given a pair of (not necessarily distinct) nodes $u, w$, we
define $\cP_{(u,w)}$ as the set $\{v ~:~ p_{T_1}(v) = u, ~ p_{T_2}(v)
= w\}$. Since each vertex different from the root has exactly one
parent in each tree, each vertex $v\in [n]$ belongs to exactly one set $\cP_{(u,w)}$.
We then denote by $\cP$ the collection of the nonempty sets $\cP_{(u,w)}$, $u,w \in V$ such that $u\neq w$, and by $|\cP|$ the number of sets in $\cP$.
\end{comment}
Given two trees $T_1$ and $T_2$, we can associate with $v$ the pair
$(p_{T_1}(v),p_{T_2}(v))$ of the parents of $v$ in the two trees.
In fact, we collect those of such pairs that have $p_{T_1}(v)\neq p_{T_2}(v)$ into the \emph{family partition} of the two trees. Formally, we define the family partition $P(\Tone,\Ttwo)=\{(p_{T_1}(v),p_{T_2}(v)):v\neq \text{root}\land p_{T_1}(v)\neq p_{T_2}(v)\}$.
We start by giving a lemma which states that when we
apply a permutation to the labels of $T_1$ obtaining $T'_1$, the size
of the resulting set $P(\Tone',\Ttwo)$ cannot increase or decrease
too much with respect to the size of $P(\Tone,\Ttwo)$.

\begin{lemma}
  \label{lemma:partition}
  Given trees $T_1$ and $T_2$, if $T_1'$ is the tree (isomorphic to $T_1$)
  resulting from the application of permutation $\pi$ of the labels of $T_1$, then $|P(\Tone,\Ttwo)| - 2|\pi|
  \le |P(\Tone',\Ttwo)| \le |P(\Tone,\Ttwo)| + 2|\pi|$.
\end{lemma}

\begin{proof}
  Let $a$ be some node of $T_1$ which has been perturbed by
  permutation $\pi$, i.e., $\pi(a) = b \ne a$.  The new set $P(\Tone',\Ttwo)$ is obtained from $P(\Tone,\Ttwo)$ by means of deletions,
  insertions and substitutions of pairs. The crucial observation is
  that such an operation will only affect the \emph{neighborhood} of
  $a$ in $\Tone$, namely the (possibly empty) set of its children $c_{T_1}(a)$
  and its parent $p_{T_1}(a)$. 
  Consider the nodes in $c_{T_1}(a)$. They can be split into three (possibly empty) sets:
  \begin{itemize}
  \item $\{v\in c_{T_1}(a): p_{T_2}(v)=b\}$. Since $\pi$ makes $b$ the new parent of all nodes in this set, $(a,b)$ must be removed from $P(\Tone,\Ttwo)$ to obtain $P(\Tone',\Ttwo)$
  \item $\{v\in c_{T_1}(a): p_{T_2}(v)=a\}$. If this set is nonempty, then $(b,a)$ must be added to $P(\Tone,\Ttwo)$ to obtain $P(\Tone',\Ttwo)$ (note that $(b,a)$ could already be part of $P(\Tone,\Ttwo)$)
  \item $\{v\in c_{T_1}(a): p_{T_2}(v)=c\notin \{a,b\}\}$.
  The effect of $\pi$ on these nodes is to change their parent from $a$ to $b$, therefore $(a,c)\in P(\Tone,\Ttwo)$ must be replaced by  $(b,c)$ to obtain $P(\Tone',\Ttwo)$
  \end{itemize}
  %Overall, for each label $a$, at most one set of $P(\Tone,\Ttwo)$ will be created and at most one will be deleted.
  Consider now the possible effects of $\pi$ on $p_{T_1}(a)$.
  There are two cases: 
  \begin{itemize}
      \item $p_{T_1}(a)=p_{T_2}(a)$. Let $c$ such that $\pi(c)=a\neq c$. Then the new pair  $(p_{T_1}(c),p_{T_2}(a))$ must be added to $P(\Tone,\Ttwo)$ to obtain $P(\Tone',\Ttwo)$ (unless it holds that $p_{T_1}(c)=p_{T_1}(a)$)
      \item $p_{T_1}(a)\neq p_{T_2}(a)$. Then $(p_{T_1}(a),p_{T_2}(a))\in P(\Tone,\Ttwo)$ and it must be removed if $p_{T_1}(c)=p_{T_2}(a)$, or it must be replaced by $(p_{T_1}(c),p_{T_2}(a))$ otherwise
   \end{itemize} 
   \begin{comment}
  Notice that $a\in \cP_{(p_{T_1}(a),p_{T_2}(a))}$ and $a\in \cP'_{(p_{T_1}(b),p_{T_2}(a))}$.
  Therefore $\cP_{(p_{T_1}(a),p_{T_2}(a))}$ is the only set of $\cP$ that might be deleted, while $\cP'_{(p_{T_1}(b),p_{T_2}(a))}$ is the only set that might be created.
    \end{comment}
  In summary, $P(\Tone',\Ttwo)$ is obtained from $P(\Tone,\Ttwo)$ with up to two deletions
  and two additions of pairs in $P(\Tone,\Ttwo)$ (and an unbounded number of replacements) for each label
  involved in the permutation $\pi$. The result follows.
\end{proof}

In the special case where one of the trees, e.g., $T_1$, is
binary, that is, each node has up to two children, we have the
...
\begin{proof}
  ...
  $T_1$.  By the definition of $P(\Tone',\Ttwo)$, the optimal number of link-and-cut operations to obtain
  $T_2$ from $T'_1$ is at least $|P(\Tone',\Ttwo)|$; we thus have that
  $d(T_1,T_2) \ge |\pi| + |P(\Tone',\Ttwo)| $. Moreover, Lemma
  \ref{lemma:partition} says that $|\pi|\ge \frac{|P(\Tone,\Ttwo)|-|P(\Tone',\Ttwo)|}{2}$,
  thus $d(T_1,T_2)\ge \frac{|P(\Tone,\Ttwo)|-|P(\Tone',\Ttwo)|}{2} + |P(\Tone',\Ttwo)| =
  \frac{|P(\Tone,\Ttwo)|}{2} + \frac{|P(\Tone',\Ttwo)|}{2} \ge \frac{|P(\Tone,\Ttwo)|}{2}$. Let $X$ be the set nodes whose parents in $\Tone$ and $\Ttwo$ are different. Now, since
  $T_1$ is binary, each pair  $(x,y)\in P(\Tone,\Ttwo)$ corresponds to
  up to two elements (the children of $x$ in $T_1$ that become the children of $y$ in
  $T_2$), thus they cannot be more than the number of children of
  $x$. It follows that $|X|=d_{\ell}(T_1,T_2)\le 2\cdot|P(\Tone,\Ttwo)|$,
...
binary. Together with Lemma~\ref{lemma:algorithm-link-and-cut}, we obtain the following theorem.
...
\section{Experimental evaluation of the approximation factor}
\label{sec:experimental}

To empirically evaluate the approximation factor of the algorithm 
described is Section \ref{sec:approxtree} we performed a series
of experiments in which we implemented the general approximation
algorithm $Approx$ and an exponential exact algorithm $Exac$.
For each experiment we generated a total of 100 random trees
of $n$ nodes (labels)
that were then perturbed with a total of $d$ randomly chosen
link-and-cut and permutation operations; the distance between
the original and the perturbed tree was then computed using
the $Approx$ and the $Exac$ algorithms.
The latter was used because, given the randomness of the operations,
it is possible for the actual distance to be less than $d$.

Table~\ref{table:experiments} reports the results for each settings
of the experimentation; the highest approximation factor reported
is 6, but the average is ~3 suggesting that the actual
expected approximation factor is lower than the reported maximum.
Interestingly to notice is the running time of the $Approx$ algorithm
that remains almost instantaneous, while the $Exac$ algorithm, 
as expected, quickly becomes unusable.
In fact we designed and run the experiment for 
$n=\{10,20,30,40,50,80,160,320\} \times d=\{3,4,5,6\}$
where the combinations not reported in the table are
omitted because the exponential-time algorithm
was not able to compute in an acceptable time frame.

\begin{table}[]
\centering
\begin{tabular}{ l|l|r|r|r|r } 
	$n$ & $d$ & Max a.f. & Median a.f. & $Approx$ run time (s) & $Exac$ run time (s) \\\hline
	10 & 3 & 5.00 & 2.00 & 1.86e-04 & 1.02e-03 \\
	10 & 4 & 6.00 & 1.75 & 1.79e-04 & 1.09e-02 \\
	10 & 5 & 4.00 & 1.60 & 1.79e-04 & 8.04e-02 \\
	10 & 6 & 3.50 & 1.40 & 1.73e-04 & 3.52e-01 \\
	20 & 3 & 5.00 & 3.00 & 3.57e-04 & 7.01e-03 \\
	20 & 4 & 5.00 & 2.50 & 3.62e-04 & 1.86e-01 \\
	20 & 5 & 3.80 & 2.60 & 3.52e-04 & 3.95e+00 \\
	20 & 6 & 4.25 & 2.33 & 3.46e-04 & 5.87e+01 \\
	30 & 3 & 5.33 & 3.00 & 5.80e-04 & 2.14e-02 \\
	30 & 4 & 4.33 & 3.00 & 5.65e-04 & 8.42e-01 \\
	30 & 5 & 4.25 & 3.00 & 5.72e-04 & 2.85e+01 \\
	30 & 6 & 4.40 & 2.83 & 5.65e-04 & 9.40e+02 \\
	40 & 3 & 5.00 & 3.33 & 8.56e-04 & 4.71e-02 \\
	40 & 4 & 5.00 & 3.25 & 8.43e-04 & 2.62e+00 \\
	40 & 5 & 5.00 & 3.20 & 8.33e-04 & 1.29e+02 \\
	50 & 3 & 5.33 & 3.33 & 1.18e-03 & 8.67e-02 \\
	50 & 4 & 5.75 & 3.25 & 1.17e-03 & 5.89e+00 \\
	50 & 5 & 4.50 & 3.20 & 1.16e-03 & 3.93e+02 \\
	80 & 3 & 5.33 & 3.67 & 2.43e-03 & 3.25e-01 \\
	80 & 4 & 5.00 & 3.50 & 2.40e-03 & 3.56e+01 \\
	160 & 3 & 5.33 & 3.67 & 7.78e-03 & 2.37e+00 \\
	160 & 4 & 5.25 & 4.00 & 7.83e-03 & 5.77e+02 \\
	320 & 3 & 5.67 & 3.67 & 2.81e-02 & 1.82e+01 \\
\end{tabular}
\caption{Experimental results for 100 pairs of randomly generated trees of $n$ nodes (labels)
with a maximum distance of $d$ random link-and-cut or permutation operations.
The table reports the maximum approximation factor and its median
across the different settings as well as running times of both approximation and exponential
exact algorithm.}
